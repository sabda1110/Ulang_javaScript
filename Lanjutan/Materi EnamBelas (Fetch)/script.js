...
tombol.addEventListener('click', function () {
  let input = document.querySelector('.input-keyword');
  fetch(`http://www.omdbapi.com/?s=${input.value}&apikey=58dec134`)
    ...
    .then((response) => {
      ...
      const tombol = document.getElementsByClassName('exampleModal-tombol');
      for (const a of tombol) {
        a.addEventListener('click', function () {
          let idImdb = this.dataset.imdbid;
          fetch(`http://www.omdbapi.com/?i=${idImdb}&apikey=58dec134`)
            .then((response) => response.json())
            .then((response) => {
              let isiDetail = showMovieDetail(response);
              dataDetail.innerHTML = isiDetail;
            });
        });
      }
